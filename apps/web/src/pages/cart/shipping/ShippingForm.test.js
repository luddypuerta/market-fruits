...
describe("ShippingForm", () => {
    ...
    it("should show warning if any field is empty", () => {
        render(_jsx(ShippingForm, { open: true, onClose: mockOnClose, onConfirm: mockOnConfirm, showMessage: mockShowMessage }));
    ...
    it("should show error if country is not in America", async () => {
        validateCountryInAmerica.mockResolvedValue(false);
        render(_jsx(ShippingForm, { open: true, onClose: mockOnClose, onConfirm: mockOnConfirm, showMessage: mockShowMessage }));
        fillForm();
        fireEvent.click(screen.getByRole("button", { name: /confirmar/i }));
        await waitFor(() => {
            expect(mockShowMessage).toHaveBeenCalledWith("❌ El país ingresado no pertenece al continente americano.", "error");
        });
        expect(mockOnConfirm).not.toHaveBeenCalled();
    });
    it("should call onConfirm and show success message if form is valid", async () => {
        validateCountryInAmerica.mockResolvedValue(true);
        render(_jsx(ShippingForm, { open: true, onClose: mockOnClose, onConfirm: mockOnConfirm, showMessage: mockShowMessage }));
        fillForm();
        fireEvent.click(screen.getByRole("button", { name: /confirmar/i }));
        await waitFor(() => {
            ...
            expect(mockShowMessage).toHaveBeenCalledWith("✅ País válido. Pedido confirmado.", "success");
        });
    });
});
